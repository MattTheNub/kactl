...
ll MinCostMatching(const vector<vd>& cost, vi& L, vi& R) {
	...
	krep(i,0,n) {
		u[i] = cost[i][0];
		krep(j,1,n) u[i] = min(u[i], cost[i][j]);
	}
	krep(j,0,n) {
		v[j] = cost[0][j] - u[0];
		krep(i,1,n) v[j] = min(v[j], cost[i][j] - u[i]);
	}

	/// find primal solution satisfying complementary slackness
	L = vi(n, -1);
	R = vi(n, -1);
	krep(i,0,n) krep(j,0,n) {
		if (R[j] != -1) continue;
...
void testMatching() {
	krep(it,0,100000) {
		...
		krep(i,0,N) krep(j,0,M) co[i][j] = (rand() % 10) + 2;
		vi L, R;
		ll v = MinCostMatching(co, L, R);
		int S = N+M, T = N+M+1;
		MCMF mcmf(N+M+2);
		krep(i,0,N) mcmf.addEdge(S, i, 1, 0);
		krep(i,0,M) mcmf.addEdge(N+i, T, 1, 0);
		krep(i,0,N) krep(j,0,M) mcmf.addEdge(i, N+j, 1, co[i][j] - 2);
		mcmf.setpi(S);
		auto pa = mcmf.maxflow(S, T);
...
void testNeg() {
	...
	krep(it,0,ITS) {
		...
		MCMF mcmf(N);
		MCMF2 mcmf2(N);
		krep(i,0,N) krep(j,0,N) ed[i][j] = 0;
		bool anyneg = false;
		krep(eid,0,M) {
