...
int matInv(vector<vector<ll>>& A) {
	...
	krep(i,0,n) {
		int r = i, c = i;
		krep(j,i,n) krep(k,i,n) if (A[j][k]) {
			r = j; c = k; goto found;
		}
		return i;
found:
		...
		krep(j,i+1,n) {
			ll f = A[j][i] * v % mod;
			A[j][i] = 0;
			krep(k,i+1,n) A[j][k] = (A[j][k] - f*A[i][k]) % mod;
			krep(k,0,n) tmp[j][k] = (tmp[j][k] - f*tmp[i][k]) % mod;
		}
		krep(j,i+1,n) A[i][j] = A[i][j] * v % mod;
		krep(j,0,n) tmp[i][j] = tmp[i][j] * v % mod;
		A[i][i] = 1;
	}

	for (int i = n-1; i > 0; --i) krep(j,0,i) {
		ll v = A[j][i];
		krep(k,0,n) tmp[j][k] = (tmp[j][k] - v*tmp[i][k]) % mod;
	}
